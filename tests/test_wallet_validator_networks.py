...
def test_supported_networks_are_explicitly_validated():
    assert WalletValidator.SUPPORTED_NETWORKS == {"BEP20", "TRC20", "ARB", "SOLANA", "ETH", "POLYGON"}
...
def test_network_aliases_are_canonicalized():
    ...
    assert WalletValidator.normalize_network("MATIC") == "POLYGON"
    assert WalletValidator.normalize_network("POL") == "POLYGON"
