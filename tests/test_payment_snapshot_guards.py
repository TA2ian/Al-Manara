...
class PaymentSnapshotGuardTests(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self):
        ...
        async with self.pool.acquire() as conn:
            ...
            await conn.execute("""
                CREATE TABLE users (id SERIAL PRIMARY KEY, telegram_id BIGINT UNIQUE NOT NULL)
            """)
            await conn.execute("""
            ...
            await conn.execute("""
                CREATE TABLE payment_methods (
                    id SERIAL PRIMARY KEY, code TEXT UNIQUE NOT NULL, provider TEXT NOT NULL,
                    currency TEXT NOT NULL, account_identifier TEXT NOT NULL DEFAULT '', qr_photo_id TEXT,
    ...
    async def _insert_order(self):
        async with self.pool.acquire() as conn:
            return await conn.fetchrow("""INSERT INTO orders
                ...
                VALUES (1,'0xPAY','BEP20','wallet-qr','NEW.SYP')
                RETURNING *""")
    ...
    async def test_snapshot_is_immutable(self):
        ...
        with self.assertRaises(Exception):
            async with self.pool.acquire() as conn:
                await conn.execute(
                    "UPDATE orders SET payment_account_snapshot='ACCOUNT-B' WHERE id=$1",
                    row["id"],
                )
    ...
    async def test_changed_payment_method_does_not_change_existing_order(self):
        ...
        async with self.pool.acquire() as conn:
            await conn.execute(
                "UPDATE payment_methods SET account_identifier='ACCOUNT-B', qr_photo_id='payment-qr-b' WHERE code='shamcash_new_syp'"
            )
            current = await conn.fetchrow(
                "SELECT payment_account_snapshot,payment_qr_photo_id FROM orders WHERE id=$1", row["id"]
            )
