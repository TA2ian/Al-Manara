...
from handlers import admin_approval_policy, order_amount_policy, order_confirmation_policy
...
def test_payment_timeout_is_loaded_from_all_new_order_paths():
    confirmation_source = inspect.getsource(order_confirmation_policy)
    approval_source = inspect.getsource(admin_approval_policy)
    for source in (confirmation_source, approval_source):
        assert "SettingsService" in source
