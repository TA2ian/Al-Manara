...
CURRENCY_META = {
    ...
    "NEW.SYP": "الليرة السورية الجديدة",
}
...
async def payment_method_view(callback: CallbackQuery):
    ...
    async with pool.acquire() as conn:
        method = await conn.fetchrow(
            """SELECT code, currency, recipient_name, account_identifier, qr_photo_id, enabled
                 FROM payment_methods WHERE code=$1 AND provider='ShamCash'""", code
        )
    if not method:
...
async def payment_method_recipient_save(message: Message, state: FSMContext):
    ...
    await message.answer(
        _progress(2, "عنوان الاستلام", "أرسل عنوان/رقم حساب ShamCash الذي سيستقبل الدفعة."),
        reply_markup=_cancel_keyboard(), parse_mode="HTML",
    )
...
async def payment_method_address_save(message: Message, state: FSMContext):
    ...
    await message.answer(
        _progress(3, "رمز QR", "أرسل الآن صورة QR الخاصة بنفس عنوان الاستلام.\n\nسيتم قراءة العنوان من QR ومقارنته بالعنوان الذي أدخلته قبل الحفظ."),
        reply_markup=_cancel_keyboard(), parse_mode="HTML",
    )
...
async def payment_method_qr_receive(message: Message, state: FSMContext):
    ...
    if qr_address.casefold() != address.casefold():
        await message.answer(
            ...
            "أعد إرسال QR المطابق.", parse_mode="HTML"
        )
        return
    ...
    async with pool.acquire() as conn:
        row = await conn.fetchrow(
            """UPDATE payment_methods
                  SET recipient_name=$1, account_identifier=$2, qr_photo_id=$3, updated_at=NOW()
                WHERE code=$4 AND provider='ShamCash'
            RETURNING currency""",
        ...
        await conn.execute(
            ...
            f"payment_method={code}; currency={row['currency']}; qr_address_verified=true",
    ...
    async with pool.acquire() as conn:
        row = await conn.fetchrow(
            """UPDATE payment_methods SET enabled=NOT enabled, updated_at=NOW()
                WHERE code=$1 AND provider='ShamCash' RETURNING currency, enabled""", code
        )
        if not row:
            await callback.answer("وسيلة الدفع غير موجودة", show_alert=True)
            return
