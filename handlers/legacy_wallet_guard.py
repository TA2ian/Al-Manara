"""Safety guard for legacy per-order wallet QR states.

All wallet registration now belongs to the verified wallet registry. This
router runs before the legacy order router and prevents stale FSM sessions
from reaching the old per-order QR handlers.
"""
...
async def _redirect(message: Message | CallbackQuery, state: FSMContext, telegram_id: int):
    ...
    text = (
        ...
        "This was a legacy order session that requested a QR inside the order. That path is disabled.\n\n"
    ...
    if isinstance(message, CallbackQuery):
        await message.message.edit_text(text, reply_markup=_wallet_keyboard(lang), parse_mode="HTML")
        await message.answer()
    else:
        await message.answer(text, reply_markup=_wallet_keyboard(lang), parse_mode="HTML")
...
async def block_legacy_wallet_qr_callbacks(callback: CallbackQuery, state: FSMContext):
    """Block every callback from the retired per-order QR state."""
...
async def block_legacy_wallet_qr_messages(message: Message, state: FSMContext):
    """Block every message/photo from the retired per-order QR state."""
