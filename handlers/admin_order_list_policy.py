...
async def _fetch_orders_page(list_type: str, page: int = 0):
    ...
    async with pool.acquire() as conn:
        ...
        rows = await conn.fetch(
            "SELECT o.*, u.full_name, u.telegram_id AS user_tg "
...
def _format_order_compact(order, show_detail: bool = False) -> str:
    ...
    if show_detail:
        return (
            ...
            f"💰 {order['amount_usdt']} USDT | 🌐 {html.escape(order['network'] or '')}\n"
            f"💱 {html.escape(order['payment_currency'] or '')} | 💵 {order['total_amount']:.2f}\n"
    ...
    return (
        ...
        f"💰 {order['amount_usdt']} USDT | 🌐 {html.escape(order['network'] or '')}\n"
        f"💱 {order['total_amount']:.2f} {html.escape(order['payment_currency'] or '')}\n"
        f"📍 <code>{wallet_short}</code>\n"
        f"📅 {order['created_at'].strftime('%m-%d %H:%M')}"
    )
...
async def _render_orders(callback: CallbackQuery, list_type: str, page: int):
    ...
    if list_type == "pending":
        header = f"📦 <b>الطلبات المعلقة</b> ({total}) | صفحة {page + 1}/{total_pages}"
    else:
        header = (
    ...
    for order in rows:
        await callback.message.answer(
            _format_order_compact(order, show_detail=list_type == "active"),
    ...
    await callback.message.answer(
        ...
        reply_markup=order_pagination_keyboard(page, total_pages, list_type),
        parse_mode="HTML",
    )
...
async def pending_orders(callback: CallbackQuery):
    if not is_admin(callback.from_user.id):
        ...
        return
    await _render_orders(callback, "pending", 0)
...
async def pending_orders_page(callback: CallbackQuery):
    ...
    try:
        ...
        return
    await _render_orders(callback, "pending", page)
    await callback.answer()
...
async def active_orders(callback: CallbackQuery):
    if not is_admin(callback.from_user.id):
        await callback.answer("⛔ Access denied", show_alert=True)
        return
    await _render_orders(callback, "active", 0)
    await callback.answer()
...
async def active_orders_page(callback: CallbackQuery):
    ...
    try:
        ...
        await callback.answer("❌ صفحة غير صالحة", show_alert=True)
        return
    await _render_orders(callback, "active", page)
    await callback.answer()
