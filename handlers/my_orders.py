...
async def _notify_admins_receipt(
    ...
    """Send the receipt and a consistently formatted review summary to admins."""
...
async def handle_receipt_upload(message: Message, state: FSMContext):
    """Process one receipt upload while preserving the order transition graph."""
    ...
    auto_verified = bool(verification_result and verification_result.get("auto_verified"))
        ...
        if auto_verified or remaining_attempts <= 0:
            try:
                updated = await transition_order(
                    ...
                    order_id,
                    "receipt_received",
                    updates={"receipt_photo_id": photo_id, "receipt_upload_count": attempt_count},
                )
    ...
    if auto_verified:
        ...
        await _notify_admins_receipt(bot, dict(updated), photo_id, verification_result, message.from_user.username or "", True)
        await state.clear()
        return
    ...
    score = verification_result.get("score", 0) if verification_result else 0
    label = verification_result.get("score_label", "فاشل") if verification_result else "فاشل"
    await message.answer(
