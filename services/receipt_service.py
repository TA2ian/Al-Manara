...
async def notify_admins_receipt(bot: Bot, order: dict, photo_id: str, verification_result: dict | None, username: str, is_auto_verified: bool) -> None:
...
async def handle_receipt_upload(message: Message, state: FSMContext) -> None:
    ...
    async with pool.acquire() as conn:
        order = await conn.fetchrow(
            """SELECT o.*, u.telegram_id AS user_telegram_id, u.full_name, u.shamcash_account, u.language
    ...
    async with receipt_processing_lock(int(order_id)) as acquired:
        ...
        try:
            try:
                ...
                image_bytes, _ = normalize_receipt_media(image_file.read(), "image/jpeg", "receipt.jpg")
            except ValueError as exc:
                await message.answer(f"❌ ملف الإيصال غير صالح: {html.escape(str(exc))}", parse_mode="HTML")
                return
            except Exception:
                logger.exception("Failed to download or normalize receipt image for order %s", order_id)
                await message.answer("❌ تعذر قراءة صورة الإيصال. أعد إرسال صورة واضحة.")
                return
            ...
            async with pool.acquire() as conn:
                ...
                if auto_verified or remaining_attempts <= 0:
                    try:
                        updated = await transition_order(conn, order_id, "receipt_received", updates={"receipt_photo_id": photo_id, "receipt_upload_count": attempt_count})
            ...
            if auto_verified:
                ...
                await message.answer(f"✅ <b>تم التحقق من الإيصال آلياً.</b>\n\n📊 نسبة التطابق: <b>{score}%</b> ({html.escape(str(label))})\n📦 تم إرسال الإيصال للمراجعة النهائية من الإدارة.", parse_mode="HTML")
                await notify_admins_receipt(bot, dict(updated), photo_id, verification_result, message.from_user.username or "", True)
                await state.clear()
                return
            ...
            if remaining_attempts <= 0:
                ...
                await notify_admins_receipt(bot, dict(updated), photo_id, verification_result, message.from_user.username or "", False)
                await state.clear()
                return

            score = verification_result.get("score", 0)
            label = verification_result.get("score_label", "فاشل")
            await message.answer(f"⚠️ <b>تعذر التحقق من الإيصال آلياً.</b>\n\n📊 نسبة التطابق: <b>{score}%</b> ({html.escape(str(label))})\n\n🔄 لديك <b>{remaining_attempts}</b> محاولات متبقية. يمكنك إعادة الرفع أو اختيار المراجعة اليدوية.", parse_mode="HTML")
