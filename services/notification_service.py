...
def _html(value: object) -> str:
    """Escape dynamic content before embedding it in Telegram HTML."""
...
class NotificationService:
    ...
    async def notify_feedback(self, user: dict, text: str, attachment_type: str | None = None, attachment_file_id: str | None = None):
        ...
        body = (
            ...
            f"🆔 ID: <code>{_html(user_id)}</code>\n"
            f"📝 الرسالة: {_html(text or 'بدون نص')}"
        )
        for admin_id in self._admin_ids:
            try:
    ...
    async def notify_new_order(self, order: dict):
        text = (
            "📦 <b>طلب جديد!</b>\n\n"
            f"📋 الرقم: #{_html(order.get('order_number', 'N/A'))}\n"
            f"👤 العميل: {_html(order.get('username', 'N/A'))}\n"
            f"💰 المبلغ: {_html(usdt(order['amount_usdt']))} USDT\n"
            f"🌐 الشبكة: {_html(order['network'])}\n"
            f"💱 العملة: {_html(order['payment_currency'])}"
        )
        ...
        if not recipient or not address or not qr_photo_id:
            ...
            try:
                await self.notify_user(
                    user_id,
                    f"⚠️ <b>تعذر تجهيز بيانات الدفع للطلب #{_html(order_number)}</b>\n\nبيانات الدفع المثبتة لهذا الطلب غير مكتملة. <b>لا ترسل أي مبلغ.</b> يرجى مراجعة الإدارة."
                    if lang == "ar" else
                    f"⚠️ <b>Payment details unavailable for order #{_html(order_number)}</b>\n\nThe immutable payment details for this order are incomplete. <b>Do not send funds.</b> Please contact administration.",
                )
            except Exception:
                logger.exception("Failed to send incomplete-payment warning for %s", order_number)
            return False

        safe_recipient = _html(recipient)
        safe_address = _html(address)
        safe_order_number = _html(order_number)
        safe_currency = _html(currency)
        safe_amount = _html(amount)
        safe_fee_amount = _html(fee_amount)
        safe_fee_percent = _html(fee_percent)
        old_syp_line = ""
        if currency == "NEW.SYP":
            old_syp_amount = order.get("old_syp_total")
            if old_syp_amount is not None:
                old_syp_line = f"\nℹ️ يعادل <b>{_html(money(old_syp_amount))}</b> ليرة سورية قديمة" if lang == "ar" else f"\nℹ️ Equivalent to <b>{_html(money(old_syp_amount))}</b> legacy Syrian pounds"

        if lang == "ar":
            caption = (
                f"🔔 <b>بيانات الدفع الرسمية · الطلب #{safe_order_number}</b>\n\n"
                "تم اعتماد الطلب. هذه هي بيانات الدفع المثبتة لهذا الطلب، ولا تستخدم أي بيانات من رسالة أخرى.\n\n"
                "💳 <b>الدفع إلى</b>\n"
                f"👤 المستلم: <b>{safe_recipient}</b>\n"
                f"📍 العنوان/الحساب: <code>{safe_address}</code>\n"
                f"💱 العملة: <b>{safe_currency}</b>\n"
                f"💰 المبلغ المستحق: <b>{safe_amount} {safe_currency}</b>\n"
                f"🏷️ رسوم الخدمة: <b>{safe_fee_amount} {safe_currency}</b> ({safe_fee_percent}%)"
                f"{old_syp_line}\n\n"
                "⏱️ <b>مهلة إتمام الدفع</b>\n"
                f"لديك <b>{_html(deadline_minutes)} دقيقة</b> من اعتماد الطلب لإتمام الدفع وإرسال الإثبات.\n\n"
                "🔎 <b>قبل التحويل</b>\n"
                "تحقق من اسم المستلم والعنوان مرة أخيرة. بعد إتمام الدفع ارفع الإثبات من داخل هذا الطلب."
            )
        else:
            caption = (
                f"🔔 <b>Official payment details · Order #{safe_order_number}</b>\n\n"
                "Your order is approved. These are the payment details locked to this order; do not use details from another message.\n\n"
                "💳 <b>Pay to</b>\n"
                f"👤 Recipient: <b>{safe_recipient}</b>\n"
                f"📍 Address/account: <code>{safe_address}</code>\n"
                f"💱 Currency: <b>{safe_currency}</b>\n"
                f"💰 Amount due: <b>{safe_amount} {safe_currency}</b>\n"
                f"🏷️ Service fee: <b>{safe_fee_amount} {safe_currency}</b> ({safe_fee_percent}%)\n\n"
                "⏱️ <b>Payment deadline</b>\n"
                f"You have <b>{_html(deadline_minutes)} minutes</b> from order approval to complete the payment and upload the proof.\n\n"
                "🔎 <b>Before sending</b>\n"
