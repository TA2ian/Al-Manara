...
def preset_amounts_keyboard(lang: str = "ar") -> InlineKeyboardMarkup:
    ...
    for i in range(0, len(amounts), 2):
        rows.append([InlineKeyboardButton(text=f"{amount} USDT", callback_data=f"amount_preset_{amount}") for amount in amounts[i:i + 2]])
...
def orders_pagination_keyboard(current_page: int, total_pages: int, lang: str = "ar") -> InlineKeyboardMarkup:
    ...
    nav = []
    if current_page > 1:
        nav.append(InlineKeyboardButton(text="⬅️ السابق" if lang == "ar" else "⬅️ Previous", callback_data=f"orders_page_{current_page - 1}"))
    if current_page < total_pages:
        nav.append(InlineKeyboardButton(text="التالي ➡️" if lang == "ar" else "Next ➡️", callback_data=f"orders_page_{current_page + 1}"))
    if nav:
        buttons.append(nav)
