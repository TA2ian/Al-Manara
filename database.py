...
async def init_db():
    ...
    async with _pool.acquire() as conn:
        ...
        if legacy_method:
            if canonical_syp:
                if not canonical_syp["account_identifier"] and legacy_method["account_identifier"]:
                    await conn.execute("UPDATE payment_methods SET account_identifier = $1, updated_at = NOW() WHERE id = $2", legacy_method["account_identifier"], canonical_syp["id"])
                if not canonical_syp["qr_photo_id"] and legacy_method["qr_photo_id"]:
                    await conn.execute("UPDATE payment_methods SET qr_photo_id = $1, updated_at = NOW() WHERE id = $2", legacy_method["qr_photo_id"], canonical_syp["id"])
        ...
        await conn.execute("""
            CREATE OR REPLACE FUNCTION enforce_canonical_payment_method_row() RETURNS TRIGGER AS $$
            BEGIN
                IF NEW.provider = 'ShamCash' AND NEW.code NOT IN ('shamcash_usd', 'shamcash_new_syp') THEN RAISE EXCEPTION 'non-canonical ShamCash payment method code: %', NEW.code USING ERRCODE='23514'; END IF;
                IF NEW.provider = 'ShamCash' AND NEW.currency NOT IN ('USD', 'NEW.SYP') THEN RAISE EXCEPTION 'non-canonical ShamCash payment currency: %', NEW.currency USING ERRCODE='23514'; END IF;
                RETURN NEW;
        ...
        if count == 0:
            await conn.execute("INSERT INTO exchange_rates (rate, rate_currency, updated_by) VALUES (1, 'NEW.SYP', NULL)")
